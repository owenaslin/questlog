 ...
 * same three appear all day and rotate at midnight. Completed quests are
 * excluded so returning users always see fresh options.
 *
 * Layout: side · main · side (main quest in the centre card).
 */
export function getDailyQuests(excludeCompletedIds: string[] = [], count = 3): Quest[] {
  const today = new Date().toISOString().slice(0, 10); // e.g. "2026-04-21"
  ...
  const eligible = ALL_QUESTS.filter((q) => !excludeCompletedIds.includes(q.id));
  const shuffled = [...eligible]
    .map((q, i) => ({ q, sort: rand(i) }))
    .sort((a, b) => a.sort - b.sort)
    .map(({ q }) => q);

  // Try to give variety: one main flanked by two sides
